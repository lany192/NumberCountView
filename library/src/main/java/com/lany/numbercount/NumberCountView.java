...
public class NumberCountView extends RelativeLayout implements View.OnTouchListener, ValueAnimator.AnimatorListener, ValueAnimator.AnimatorUpdateListener {
    ...
    private void initViews(AttributeSet attrs) {
        ...
        if (attrs != null) {
            ...
            int modeValue = a.getInt(R.styleable.NumberCountView_mode, Mode.AUTO.getValue());
            mode = Mode.valueOf(modeValue);
            minValue = a.getInt(R.styleable.NumberCountView_min, minValue);
            maxValue = a.getInt(R.styleable.NumberCountView_max, maxValue);
            value = valueRangeCheck(a.getInt(R.styleable.NumberCountView_value, value));
            valueSlowStep = a.getInt(R.styleable.NumberCountView_step, valueSlowStep);
            if (valueSlowStep <= 0) valueSlowStep = 1;
            text = a.getString(R.styleable.NumberCountView_text);
            ...
            leftButtonBackground = a.getDrawable(R.styleable.NumberCountView_ncv_leftButtonBackground);
            rightButtonBackground = a.getDrawable(R.styleable.NumberCountView_ncv_rightButtonBackground);
        ...
        mMinusBtn.setOnTouchListener(this);
        mPlusBtn.setOnTouchListener(this);
    ...
    public boolean onTouch(View v, MotionEvent event) {
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (v == mMinusBtn || v == mPlusBtn || animationing) break;
                //非按钮则进行移动
            ...
            case MotionEvent.ACTION_CANCEL:
                ...
                //如果是两边的按钮，分别设置为点击状态
                if (v == mMinusBtn) {
                    mMinusBtn.setPressed(false);
                    break;
                } else if (v == mPlusBtn) {
                    mPlusBtn.setPressed(false);
                    break;
                }
    ...
    static class UpdateRunnable implements Runnable {
        private WeakReference<NumberCountView> view;

        public UpdateRunnable(NumberCountView view) {
            this.view = new WeakReference<>(view);
        }

        public void run() {
            NumberCountView stepper = view.get();
            if (stepper != null) {
                stepper.updateUI();
            }
        }
